...
def generate_config(args):
    ...
    if 0 and os.path.isfile(args.config_file):
        print('Configuration file {!r} exists! Remove it manually to'
              ' override.\n'.format(args.config_file))
    else:
        from xndtools.kernel_generator.generate_config import generate_config
        include_dirs = list(set([os.path.dirname(fn)
                                 for fn in args.header_files]))
...
def generate_kernel(args):
    ...
    args : argparse.Namespace
      Specify `xnd_tools kernel` arguments: Namespace(config_file=...,
    ...
    if not os.path.isfile(args.config_file):
        print('Not a file: {!r}. Expected file path to kernel configuration'
              ' file. Exiting.'.format(args.config_file))
        return
...
def generate_module(args):
    ...
    args : argparse.Namespace
      Specify `xnd_tools kernel` arguments:
        ...
        Namespace(config_file=..., target_file=..., target_language=None,
                  package=None, kernel_source_file=None, source_dir=...)
    ...
    if not os.path.isfile(args.kernels_source_file):
        print('Not a file: {!r}. Expected file path to kernel C source file.'
              ' Exiting.'.format(args.config_file))
        return
