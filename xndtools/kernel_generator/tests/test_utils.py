...
def test_split_expression():
    ...
    assert split_expression('a[b, c], d') == ['a[b, c]', 'd']
    assert (split_expression('a, b(c), d(e, f), g(h), i')
            == ['a', 'b(c)', 'd(e, f)', 'g(h)', 'i'])
...
def test_resolve_path():
    ...
    with pytest.warns(UserWarning):
        assert (resolve_path('${TEST_RESOLVE_PATH_NOT_EXISTING}/a')
                == os.path.join('${TEST_RESOLVE_PATH_NOT_EXISTING}', 'a'))

    assert (resolve_path('${TEST_RESOLVE_PATH_NOT_EXISTING|bar}/a')
            == os.path.join('bar', 'a'))
    assert (resolve_path('${TEST_RESOLVE_PATH_NOT_EXISTING|<prefix>}/a')
            == os.path.join(sys.prefix, 'a'))
    assert (resolve_path('p/${TEST_RESOLVE_PATH_NOT_EXISTING|}/a')
            == os.path.join('p', 'a'))
