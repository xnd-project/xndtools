...
vml = pytest.importorskip("mkl_vml")
...
def test_bench_exp():
    ...
    for size in sizes:
        ...
        if 0:
            ...
            xnd_timings_nothing_inout.append(timeit(vml.nothing_inout,
                                                    (xa, xr)))
        if 0:
    ...
    if 0:
        plt.plot(sizes, xnd_timings_copy, '+',
                 label='mkl_vml.copy [XND]')
        plt.plot(sizes, xnd_timings_copy_inout, '+',
                 label='mkl_vml.copy_inout [XND]')
    if 0:
        plt.plot(sizes, xnd_timings_mycopy, '+',
                 label='mkl_vml.mycopy [XND]')
        plt.plot(sizes, xnd_timings_mycopy_inout, '+',
                 label='mkl_vml.mycopy_inout [XND]')
    if 0:
        plt.plot(sizes, xnd_timings_nothing, '+',
                 label='mkl_vml.nothing [XND]')
        plt.plot(sizes, xnd_timings_nothing_inout, '+',
                 label='mkl_vml.nothing_inout [XND]')
    if 0:
        plt.plot(sizes, xnd_timings_myexp, '+',
                 label='mkl_vml.myexp [XND]')
        plt.plot(sizes, xnd_timings_myexp_inout, 'x',
                 label='mkl_vml.myexp_inout [XND]')
    if 1:
        plt.plot(sizes, xnd_timings_exp, '+',
                 label='mkl_vml.exp(xa)->xr [XND]')
        plt.plot(sizes, xnd_timings_exp_inout, 'x',
                 label='mkl_vml.exp_inout(xa, xr) [XND]')
    if 1:
        plt.plot(sizes, numpy_timings_exp, '.',
                 label='numpy.exp(a)->r')
        plt.plot(sizes, numpy_timings_exp_inout, '.',
                 label='numpy.exp(a, out=r)')
