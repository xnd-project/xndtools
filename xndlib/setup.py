...
kernel_configuration_files = ['example-kernels.cfg',
                              ...
                              'mkl_vml-kernels.cfg',
                              'mkl_blas-kernels.cfg'][1:]

ext_modules = []
for cfg in kernel_configuration_files:
    include_dirs = [NDTYPES_ROOT, XND_ROOT, GUMATH_ROOT,
                    os.path.join(CONDA_PREFIX, 'include')]
    library_dirs = [NDTYPES_ROOT, XND_ROOT, GUMATH_ROOT,
...
def mkl_blas_ext():
    ...
    include_dirs = [
        NDTYPES_ROOT, XND_ROOT, GUMATH_ROOT,
        os.path.join(CONDA_PREFIX, 'include'),
        os.path.join(MKLROOT, 'include')]
    library_dirs = [NDTYPES_ROOT, XND_ROOT, GUMATH_ROOT,
                    os.path.join(CONDA_PREFIX, 'lib'),
                    os.path.join(MKLROOT, 'lib')]
    libraries = ["ndtypes", "xnd", "gumath"]
    libraries += ['mkl_intel_ilp64', 'mkl_sequential', 'mkl_core',
                  'pthread', 'm', 'dl']
    extra_compile_args = ["-Wextra", "-Wno-missing-field-initializers",
                          "-std=c11", '-DMKL_ILP64', '-m64']
    extra_link_args = ['-Wl,--no-as-needed']
    runtime_library_dirs = []
    return Extension(
      "mkl_blas",
...
setup(
    ...
    long_description="""\
    XND Libraries provides a collection of extension modules
